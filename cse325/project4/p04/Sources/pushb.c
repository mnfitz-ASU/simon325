...
void PushBDebouncer(PushB* ioPushB)
{
	...
	// See if the switch is pressed, shorting the pull-up to ground (meaning 0)
	const bool isPressed = ((inputPin & pdir) == 0);
	// Switch state is unsettled if its different from the last time we saw it
	const bool isBouncing = (ioPushB->mIsPressed != isPressed);
	if (isBouncing)
	{
		// Only declare the button as pressed when we see that it has remained 
		// at a consistent value for kBounceCount times (where times = 5ms)
	...
	{
		// Switch state is still unsettled reset bounce count
		ioPushB->mDebounceCounter = kBounceCount;
	}
}
