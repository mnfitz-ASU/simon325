 ...
 void SimonSystickCallback()
{
	...
	*/
	 ...
	 // Callback running at interrupt level. Must do as little work as possible!!!
	 // Idea: Move the debounce code out into SimonPlay() so its not running at interrupt level
	 // Similar to the way I moved the winner and sequence display out of PIT Timer callback
...
void SimonPlay()
{
	...
	// start with an initial sequenceLength of 3
	simon->mSequenceLength = 3;
	
	// Run DisplayWinner for fun :)
	SimonDisplayWinner(simon);
	for (;;)
	{
		// Create a Simon LED Sequence
		SimonCreateSequence(simon);
		// Display that Simon LED Sequence
		SimonDisplaySequence(simon);
		// Read the switches to see if the user remembered the sequence
		const bool isWinner = SimonReadPushBSequence(simon);
		if (isWinner)
		{
			// If correct, display winner
			SimonDisplayWinner(simon);
		}
		else
		{
			// Else, display loser and end the game
			SimonDisplayLoser(simon);
			break;
		}
		// Make it harder by increasing the sequence by 1
